...
inlay_hints.get_callback = function(opts)
  ...
  local enabled = opts.enabled or {"ChainingHint", "TypeHint"}
  ...
  return function(err, _, result, _, bufnr)
    ...
    local display_virt_text = function(hint)
      ...
      if hint.kind == "TypeHint" then
        local symbol_start = hint.range["start"].character 
        local symbol_end = hint.range["end"].character 
        local line_text = vim.api.nvim_buf_get_lines(bufnr, end_line, end_line + 1, false)[1]
        symbol = string.sub(line_text, symbol_start+1, symbol_end) .. ": "
      end
    
      if aligned then
        local line_length = #vim.api.nvim_buf_get_lines(bufnr, end_line, end_line + 1, false)[1]
        text = string.format("%s %s", (" "):rep(longest_line - line_length), prefix .. symbol .. hint.label)
      else
        text = prefix .. symbol .. hint.label
      end
      -- 
    print("================== text =====================")
    print(text)
